...
export async function loadCommands(client) {
    ...
    logger.debug(
        `Loaded ${totalSubcommands} total subcommands`
    );
...
export async function registerCommands(
    ...
    try {
        ...
        // Check whether /moveall is being sent to Discord
        const moveAllCommand =
            commands.find(
                (cmd) =>
                    cmd.name ===
                    'moveall'
        ...
        if (moveAllCommand) {
            ...
            logger.error(
                '❌ /moveall IS NOT included in registration payload'
            );
        }
        ...
        // Verify what Discord actually registered
        try {
            const registeredCommands =
                await client.application.commands.fetch();

            const moveAllRegistered =
            ...
            if (moveAllRegistered) {
                ...
                logger.error(
                    '❌ Discord does NOT have /moveall after registration'
                );
            }

            logger.info(
                `Discord currently has ${registeredCommands.size} global commands`
            );
        } catch (verifyError) {
            logger.error(
                'Failed to verify registered Discord commands:',
                verifyError
            );
        }
    } catch (error) {
